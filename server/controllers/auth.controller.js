...
export const signinAdmin = async (req, res, next) => {
  ...
  try {
    ...
    if (!validUser) return next(ErrorsHandling(404, "Admin not found"));

    const validPassword = await bcryptjs.compare(password, validUser.password);
    if (!validPassword)
      return next(ErrorsHandling(401, "Invalid Password or email"));
...
export const createUserByAdmin = async (req, res, next) => {
  try {
    const { username, mobile, email, password, plantname, address } = req.body;
